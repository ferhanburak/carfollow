...
- `npm install`
- `npm run build`
- `npm run test:run` (`63/63` frontend tests)
- `npm run test:functions` (`12/12` backend domain tests)
- `npm run test:emulators` (`27/27` Auth, Firestore, RTDB, and Storage identity/authorization tests)
- `npm run rules:check`
...
- backend-created immutable Vehicle Passport export snapshots for resale/passport sharing
- backend-owned Vehicle Passport transfer requests with owner cancellation and audit trail
...
- Vehicle Passport resale exports are generated by Cloud Functions and clients can only read them
- Vehicle Passport ownership-transfer requests are generated/cancelled by Cloud Functions and logged as private audit events
...
requestVehiclePassportTransfer
cancelVehiclePassportTransfer
startDriveSession
finishDriveSession
```
...
- continue Stage 3 with target-side accept/decline and final ownership handoff
- add export share tokens/PDF rendering after the backend snapshot and transfer model is stable
