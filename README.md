...
- `npm install`
- `npm run build`
- `npm run test:run` (`61/61` frontend tests)
- `npm run test:functions` (`9/9` backend domain tests)
- `npm run test:emulators` (`26/26` Auth, Firestore, RTDB, and Storage identity/authorization tests)
- `npm run rules:check`
...
- continue Stage 3 with backend-owned Vehicle Passport ownership-transfer requests
- add export share tokens/PDF rendering after the backend snapshot model is stable
