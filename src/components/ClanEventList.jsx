...
function getStatusView(status) {
  const views = {
    ...
    completed: { label: "Tamamlandı", tone: "border-lime-400/20 bg-lime-400/10 text-lime-200" },
    ...
    rolling: { label: "Sürüyor", tone: "border-lime-400/20 bg-lime-400/10 text-lime-200" },
  };
  ...
  return (
    ...
    >
      <div className="min-w-0">
        ...
        <p className="mt-1 truncate text-xs text-neutral-400">{attendee.fullName || "CRUISER Driver"} / {attendee.model || "Araç bilgisi yok"}</p>
      </div>
...
function ClanEventCard({ canManage, event, isHost, isPending, onDelete, onOpenProfile }) {
  ...
  const deleteLabel = status === "planning" ? "Planlanan Eventi Sil" : "Geçmişten Sil";
  ...
  return (
    <div className="rounded-2xl border border-white/8 bg-white/[0.03] p-3">
      <button
        ...
        aria-label={`${event.name} katilimcilarini ${isExpanded ? "gizle" : "göster"}`}
      ...
      >
        ...
        <div className="mt-3 flex items-center justify-between gap-3 text-xs text-neutral-500">
          ...
          <span className="text-neutral-300">{attendees.length} katılımcı &rsaquo;</span>
        </div>
      </button>
      ...
      {isExpanded ? (
        <div className="mt-3 border-t border-white/8 pt-3">
          <div className="mb-2 flex items-center justify-between gap-3">
            <p className="text-[10px] uppercase tracking-[0.2em] text-neutral-500">Katılan Sürücüler</p>
          ...
          <div className="space-y-2">
            {attendees.length
              ...
              : <p className="rounded-xl border border-dashed border-white/10 px-3 py-4 text-center text-xs text-neutral-500">Bu etkinlik için görünür katılımcı kaydı yok.</p>}
          </div>
            ...
            <div className="mt-3">
              {confirmingDelete ? (
                <div className="rounded-xl border border-rose-400/20 bg-rose-500/[0.06] p-3">
                  <p className="text-xs leading-5 text-rose-100">Bu etkinlik ve katılımcı bağlantıları kalıcı olarak silinecek.</p>
...
export function ClanEventList({ canManage, eventPendingId, events, onDelete, onOpenProfile, userPlate }) {
  ...
  return (
    <div className="space-y-4">
      <EventGroup canManage={canManage} emptyText="Aktif veya yaklaşan etkinlik yok." events={activeEvents} eventPendingId={eventPendingId} onDelete={onDelete} onOpenProfile={onOpenProfile} title="Aktif ve Yaklaşan" userPlate={userPlate} />
      <EventGroup canManage={canManage} emptyText="Klan geçmişinde tamamlanmış etkinlik yok." events={pastEvents} eventPendingId={eventPendingId} onDelete={onDelete} onOpenProfile={onOpenProfile} title="Geçmiş" userPlate={userPlate} />
    </div>
  );
}
