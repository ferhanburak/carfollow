...
export function createClan(user, clans, payload) {
  ...
  if (!name || !tag) {
    return { nextUser: state, nextClans: clans, feedback: "Klan adı ve tag gerekli." };
  }
  ...
  if (duplicateName || duplicateTag) {
    return { nextUser: state, nextClans: clans, feedback: "Bu klan adı veya tag zaten kullaniliyor." };
  }
...
export function sendClanInvite(user, targetFriend, clans) {
  const state = normalizeClanState(user);
  if (!targetFriend) {
    return { nextUser: state, feedback: "Davet gonderilecek sürücü secilemedi." };
  }

  if (!state.clan) {
    return { nextUser: state, feedback: "Önce bir klana dahil olman gerekiyor." };
  }

  if (!["owner", "captain"].includes(state.clanRole ?? "member")) {
    return { nextUser: state, feedback: "Klan daveti gondermek için lider olmalisin." };
  }

  const clan = clans.find((entry) => entry.name === state.clan);
  if (!clan) {
    return { nextUser: state, feedback: "Aktif klan bilgisi bulunamadı." };
  }

  if ((state.sentClanInvites ?? []).some((entry) => entry.targetPlate === targetFriend.plate && entry.status === "pending")) {
    return { nextUser: state, feedback: `${targetFriend.fullName} için zaten aktif bir davet var.` };
  }
  ...
  return {
    ...
    feedback: `${targetFriend.fullName} için klan daveti gönderildi.`,
  };
}
...
export function cancelClanInvite(user, inviteId) {
  ...
  return {
    ...
    feedback: targetInvite ? `${targetInvite.targetName} için giden klan daveti iptal edildi.` : "Klan daveti kaldırıldı.",
  };
}
...
export function acceptClanInvite(user, clans, inviteId) {
  ...
  if (!invite) {
    return { nextUser: state, nextClans: clans, feedback: "Klan daveti bulunamadı." };
  }
  ...
  return {
    ...
    nextClans,
    feedback: `${invite.clanName} klanina üye oldun.`,
  };
}
