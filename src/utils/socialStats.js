...
export function buildPersonalStats(user) {
  ...
  return [
    ...
    { key: "community-kudos", label: "Topluluk Katkısı", value: `${Number(user.communityKudos ?? 0)}` },
    { key: "community-likes", label: "Alınan Beğeni", value: `${Number(user.communityEventLikesReceived ?? 0) + Number(user.communityPhotoLikesReceived ?? 0)}` },
    { key: "helpful-votes", label: "Faydalı Yorum", value: `${Number(user.communityHelpfulVotesReceived ?? 0)}` },
